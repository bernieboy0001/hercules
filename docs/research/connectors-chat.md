...
- curl -X PUT 'https://discord.com/api/v10/applications/<DISCORD_APP_ID>/guilds/<GUILD_ID>/commands' -H 'Authorization: Bot <DISCORD_BOT_TOKEN>' -H 'Content-Type: application/json' -d '[{"name":"hercules","description":"Run a HERCULES workflow","type":1,"options":[{"type":3,"name":"input","description":"Text input","required":false}]}]'   # guild commands update instantly; use /applications/<APP_ID>/commands for global
...
- curl -X POST 'https://api.resend.com/domains' -H 'Authorization: Bearer $RESEND_API_KEY' -H 'Content-Type: application/json' -H 'User-Agent: hercules/0.1' -d '{"name":"mail.<yourdomain>","region":"us-east-1","capabilities":{"sending":{"status":"enabled"},"receiving":{"status":"enabled"}}}'   # returns records[] (DKIM/SPF/MX) to add at DNS; verify capabilities shape against the live docs page
- curl -X POST 'https://api.resend.com/domains/<domain_id>/verify' -H 'Authorization: Bearer $RESEND_API_KEY' -H 'User-Agent: hercules/0.1'
- curl -X POST 'https://api.resend.com/api-keys' -H 'Authorization: Bearer $RESEND_API_KEY' -H 'Content-Type: application/json' -H 'User-Agent: hercules/0.1' -d '{"name":"hercules-prod","permission":"sending_access","domain_id":"<domain_id>"}'   # needs a full_access key
- resend login --key re_...   # or export RESEND_API_KEY; then: resend domains create --name mail.<yourdomain> --region us-east-1 ; resend domains verify <domain_id> ; resend api-keys create --name hercules-prod --permission sending_access ; resend webhooks create --endpoint https://<app>.vercel.app/api/events/resend --events email.received ; resend doctor --json
...
```
{
  "display_information": { "name": "HERCULES" },
  "features": { "bot_user": { "display_name": "HERCULES", "always_online": true } },
  "oauth_config": {
...
const { data, error } = await resend.emails.send(
  { from: "HERCULES <no-reply@mail.example.com>", to: [to], subject, html },
...
//   "Content-Type": "application/json", "User-Agent": "hercules/0.1", "Idempotency-Key": k }, body })

...
| Resource | Value |
|---|---|
| Repo | https://github.com/<your-org>/hercules (private, `main`) |
| Clerk app | **HERCULES** `app_3ImGFtOGHgLVV1SJa6VTqFeNBJ4`, dev instance `ins_3ImGFp6EL9RZZvowE5Yj2MZAXYf`, Frontend API `https://curious-cat-3256.clerk.accounts.dev`, Organizations enabled (unlimited members; `force_organization_selection` set to **false** on 2026-09-02 — the app handles org selection at `/select-org` because Clerk's forced choose-organization task did not complete reliably under automation) |
| Convex project | `hercules` on team `<your-convex-team>` — dev `fastidious-puffin-373` (`https://fastidious-puffin-373.convex.cloud`, HTTP actions at `https://fastidious-puffin-373.convex.site`), prod `content-albatross-126` |
| Convex env (dev + prod) | `CLERK_FRONTEND_API_URL`, `ENGINE_SECRET`, `APP_ORIGIN` (Convex's own copy — the schedule alarm clock's outbound `POST /api/engine/schedule-tick` runs from the Convex deployment, not from Vercel, so it needs the deployment's *own* `APP_ORIGIN` rather than inheriting Vercel's; on dev this is `http://localhost:3000`, which is unreachable from Convex's cloud — a tick just retries and falls back, and a real local schedule test needs `npx convex dev --local` or a tunnel) |
| Vercel project | `hercules` on `<your-vercel-team>` (`prj_SP3inlGaaKH8SPSgO9z5HMmYcq4x`), GitHub connected, `vercel.ts` build command |
| Vercel env | `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`, `CLERK_SECRET_KEY`, `ENGINE_SECRET`, `CREDENTIALS_KEK`, `NEXT_PUBLIC_CLERK_SIGN_IN_URL`, `NEXT_PUBLIC_CLERK_SIGN_UP_URL` (prod/preview/dev); `APP_ORIGIN` (prod = https://hercules.vercel.app, dev = http://localhost:3000); `CONVEX_DEPLOY_KEY` (production key); `CONVEX_URL` (Production = https://content-albatross-126.convex.cloud — the URL the eve services read, because `NEXT_PUBLIC_CONVEX_URL` only exists inside the Next build) |
...
1. ✅ DONE (2026-09-02) **Clerk ↔ Convex integration** — https://dashboard.clerk.com/apps/setup/convex → select **HERCULES** (Development) → **Activate Convex integration**. The Frontend API URL shown must be `https://curious-cat-3256.clerk.accounts.dev` (already set on both Convex deployments).
2. **Convex preview deploy key (optional; only for PR preview deployments)** — https://dashboard.convex.dev/t/<your-convex-team>/hercules → Project Settings → **Generate Preview Deploy Key** → `vercel env add CONVEX_DEPLOY_KEY preview --value 'preview:…' --yes`.
...
- **Telegram bot (Phase 5 check)** — create a bot with @BotFather, then Connections → Telegram → paste the token. HERCULES registers the webhook automatically when `APP_ORIGIN` is https (on localhost it stores the inbound URL and skips `setWebhook`; use the Vercel preview URL or a tunnel for inbound tests).
